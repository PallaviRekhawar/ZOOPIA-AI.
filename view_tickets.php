<?php
$conn = mysqli_connect("localhost", "root", "", "zoo_planet");
$sql = "SELECT * FROM tickets ORDER BY id DESC";
$result = mysqli_query($conn, $sql);
?>

<!DOCTYPE html>
<html>
<head>
    <title>View Tickets</title>
    <style>
        body { font-family: Arial; background: #f2f7f2; margin: 0; padding: 20px; }
        .container { max-width: 900px; margin: 20px auto; background: white; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        h2 { color: #2e7d32; text-align: center; }
        table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        th { background: #2e7d32; color: white; padding: 12px; }
        td { padding: 10px; border-bottom: 1px solid #ddd; text-align: center; }
        tr:hover { background: #f5f5f5; }
        .back { display: block; text-align: center; margin-top: 20px; color: #2e7d32; text-decoration: none; }
    </style>
</head>
<body>
<div class="container">
    <h2>🎫 All Tickets</h2>
    <table>
        <tr><th>ID</th><th>Visitor Name</th><th>Email</th><th>Adults</th><th>Children</th><th>Total</th><th>Token</th><th>Date</th></tr>
        <?php
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                    <td>{$row['id']}</td>
                    <td>{$row['visitor_name']}</td>
                    <td>{$row['visitor_email']}</td>
                    <td>{$row['adult_count']}</td>
                    <td>{$row['child_count']}</td>
                    <td>₹{$row['total']}</td>
                    <td><b>{$row['token']}</b></td>
                    <td>{$row['booked_date']}</td>
                </tr>";
            }
        } else {
            echo "<tr><td colspan='8'>No tickets booked yet.</td></tr>";
        }
        ?>
    </table>
    <a href="dashboard.php" class="back">← Back to Dashboard</a>
</div>
</body>
</html>